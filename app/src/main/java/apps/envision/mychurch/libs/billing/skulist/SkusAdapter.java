 ...
 */
package apps.envision.mychurch.libs.billing.skulist;
...
import apps.envision.mychurch.libs.billing.skulist.row.RowViewHolder;
...
public class SkusAdapter extends RecyclerView.Adapter<RowViewHolder>
    ...
    public void onButtonClicked(int position) {
        ...
        if(PreferenceSettings.isUserLoggedIn()) {
            mBillingProvider.getBillingManager().startPurchaseFlow(data.getSkuDetails());
            PreferenceSettings.setBillingOngoing(true);
        }else{
            LocalMessageManager.getInstance().send(R.id.request_login);
        }
