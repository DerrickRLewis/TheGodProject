...
import apps.envision.mychurch.ui.viewholders.CommentsViewHolder;
 ...
 */
public class PostsCommentsAdapter extends RecyclerView.Adapter< RecyclerView.ViewHolder>{
    ...
    public PostsCommentsAdapter(CommentsListener commentsListener, UserPostsListener userPostsListener) {
   ...
   /* public void setAdapter(List<Comments> data) {
     ...
     * @param data
     */
    public void setMoreAdapter(List<Comments> data) {
        Collections.reverse(data);
        items.addAll(1,data);
    ...
    public void setEditedComment(Comments comment, int position) {
        items.set(position, comment);
        this.notifyItemChanged(position);
    }

    public void updateCommentsHeader(UserPosts userPosts) {
        items.set(0, userPosts);
     ...
     * add info item
     */
    public void setHeader(UserPosts userPosts) {
        items = new ArrayList<>();
        items.add(userPosts);
        this.notifyItemInserted(items.size() - 1);
    }
    ...
    public  RecyclerView.ViewHolder onCreateViewHolder(ViewGroup viewGroup, int i) {
        ...
        switch (i) {
            case VIEW_TYPE_COMMENTS:
                View v = inflater.inflate(R.layout.comments_item, viewGroup, false);
                viewHolder = new CommentsViewHolder(v,commentsListener);
                viewHolder.itemView.setClickable(true);
                break;
            case VIEW_TYPE_HEADER:
                View vH = inflater.inflate(R.layout.user_posts_list, viewGroup, false);
                viewHolder = new UserPostsViewHolder(vH, userPostsListener);
                break;
            case VIEW_TYPE_INFO:
            ...
            case VIEW_TYPE_LOAD_MORE:
                ...
                viewHolder = new LoadMoreViewHolder(vLoadMore,commentsListener);
                break;
        }
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int i) {
        switch (holder.getItemViewType()) {
            case VIEW_TYPE_COMMENTS:
                ...
                final Comments ci = (Comments) items.get(i);
                viewHolder.bind(ci);
                break;
            case VIEW_TYPE_HEADER:
                final UserPostsViewHolder headerViewHolder = (UserPostsViewHolder) holder;
                final UserPosts userPosts = (UserPosts) items.get(i);
                headerViewHolder.bind(userPosts,i);
                break;
            case VIEW_TYPE_INFO:
    ...
    private static class HeaderViewHolder extends RecyclerView.ViewHolder {
        ...
        private void bind(UserPosts userPosts){
            //title.setText(userPosts.getTitle());
            // description.setText(userPosts.getDescription());
    ...
    private static class LoadMoreViewHolder extends RecyclerView.ViewHolder {
        ...
        private LoadMoreViewHolder(View itemView, CommentsListener commentsListener) {
            super(itemView);
            load_more = itemView.findViewById(R.id.load_more);
            load_more.setOnClickListener(v -> commentsListener.loadMore());
        }
    }
